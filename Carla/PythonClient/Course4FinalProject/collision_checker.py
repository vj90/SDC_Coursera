...
class CollisionChecker:
    def __init__(self, circle_offsets, circle_radii, weight):
        ...
        self._weight         = weight
        self._obstacle_dist_array = None
        self._collision_threshold = 0.1 
    ...
    def collision_check(self, paths, obstacles):
        ...
        Also updates the array of the minimum obstacle distance of all paths
        args:
        ...
        self._obstacle_dist_array = np.zeros(len(paths), dtype=np.float)
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                circle_locations[:, 0] = [path[0][j] + np.cos(path[2][j])*k for k in self._circle_offsets] 
                circle_locations[:, 1] = [path[1][j] + np.sin(path[2][j])*k for k in self._circle_offsets]
                # --------------------------------------------------------------
                ...
                for k in range(len(obstacles)):
                    ...
                    self._obstacle_dist_array[i] = np.min(collision_dists)
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # The exact choice of objective function is up to you.
                # TODO (VJ) ignoring this for now, since this kind of cost function makes no sense
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            ...
                            pass
                obs_dist = self._obstacle_dist_array[i]
                if obs_dist<self._collision_threshold: # avoid numerical instability
                    inv_dist = 1/self._collision_threshold
                else:
                    inv_dist = 1/obs_dist   
                score += self._weight*inv_dist**2+0
