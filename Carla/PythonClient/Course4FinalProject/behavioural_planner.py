...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        # the stop sign and transition to the next state.
            ...
            if self._stop_count == STOP_COUNTS:
                ...
                # DONE: INSERT YOUR CODE y<BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                next_goal_index = goal_index # temporarily save next goal index
                goal_index, stop_sign_found = self.check_for_stop_signs(waypoints, closest_index, goal_index)
                if stop_sign_found:
                    if goal_index == self._goal_index: # This is the same stop that we have already stopped for, so ignore it:
                        stop_sign_found = False
                        goal_index = next_goal_index # our goal index is the original goal index found by looking ahead
                    else: # a new stop sign was found
                        self._goal_index = goal_index
                        self._goal_state = waypoints[self._goal_index]  
                        self._goal_state[2] = 0.0 # set v = 0
                        self._state = DECELERATE_TO_STOP # TODO (VJ) is this the correct state to transition into? The idea is to creep to the next stop
                # --------------------------------------------------------------
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        # consideration.
        # TODO (VJ) This kind of assumes that the closest wp is in front of the ego
        ...
        while wp_index < len(waypoints) - 1:
            arc_length += ((waypoints[wp_index][0]-waypoints[wp_index+1][0])**2 + (waypoints[wp_index][1]-waypoints[wp_index+1][1])**2)**0.5
            wp_index +=1
            if arc_length >=self._lookahead:
                break
        # ------------------------------------------------------------------
    ...
    def check_for_stop_signs(self, waypoints, closest_index, goal_index):
        ...
        """
        for i in range(closest_index+1, goal_index): # if closest stop is at current index, it is too late, so start checking from next one
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        cur_len = (ego_state[0] - waypoints[i][0])**2 + (ego_state[1]-waypoints[i][1])**2
        # Note: closest point could be behind the ego
        if cur_len < closest_len:
            closest_len = cur_len
            closest_index = i
    # ------------------------------------------------------------------
    # Check if the closest index is behind the ego, if yes, then pick the next index
    if closest_index < len(waypoints)-1: # Ignore if we are already close to last waypoint
        vec_waypt = np.array([waypoints[closest_index+1][0]-waypoints[closest_index][0],waypoints[closest_index+1][1]-waypoints[closest_index][1]])
        vec_ego = np.array([ego_state[0]-waypoints[closest_index][0],ego_state[1]-waypoints[closest_index][1]])
        dotprod = np.dot(vec_ego,vec_waypt)
        if dotprod>0.0:
            closest_index +=1
            closest_len = (ego_state[0] - waypoints[closest_index][0])**2 + (ego_state[1]-waypoints[closest_index][1])**2
    # Take square root since we were comparing square of distance until now
    closest_len = closest_len**0.5
