...
class PathOptimizer:
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        # DONE: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        # # Remember that s is a list
        thetas = a*s + b/2.0*s**2 + c/3.0*s**3 + d/4.0*s**4
        return thetas
        # ------------------------------------------------------------------
    ...
    def sample_spiral(self, p):
        ...
        cost = np.cos(t_points)
        sint = np.sin(t_points)
        x_points = scipy.integrate.cumtrapz(cost,s_points,initial=0)
        y_points = scipy.integrate.cumtrapz(sint,s_points,initial=0)
